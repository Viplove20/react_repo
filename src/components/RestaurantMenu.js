...
const RestaurantMenu = () => {
  ...
  for (const keys in obj1) {
    const filterData1 = data.filter((item) => item.card.info.category === keys)
    const resData1 = {
      category: keys,
      filterData: filterData1
    }
    items.push(<RestaurantCategory key= {Math.random()} resData = {resData1} />)
  }
  ...
  //   <div>
  //   {
  //     //const categories = item.card.info.category;
  ...
  //             <h2 className="font-normal"> Category: {item.card.info.category}</h2>
  //             <h5 className="font-normal">{item.card.info.description}</h5>
  //             <h6 className="font-light">Type: {item.card.info.itemAttribute.vegClassifier}</h6>
